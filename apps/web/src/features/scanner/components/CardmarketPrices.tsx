...
export function CardmarketPrices({ price, card, manual = false }: { price: PriceResult; card: CardRecognition; manual?: boolean }) {
  ...
  return (
    <section className="rounded-2xl border border-stone-200 bg-white p-5 text-stone-900 sm:p-6" aria-label="Variantes Cardmarket">
      {selected ? (
        ...
        <VariantList card={card} price={price} manual={manual} onSelect={setSelectedId} />
      )}
    </section>
  );
}

function SelectedVariant({ cardNumber, product, onBack, manual }: { cardNumber: string; product: Product; onBack: () => void; manual: boolean }) {
  return (
    <>
...
function VariantList({ card, price, onSelect, manual }: {
  ...
  return (
    <>
      ...
      {!manual && <p className="mt-3 text-sm text-stone-700">
      ...
      <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
        {price.products?.map(product => (
          <VariantChoice key={product.id} product={product} manual={manual} onSelect={onSelect} />
        ))}
      </div>
    </>
  );
...
function VariantChoice({ product, onSelect, manual }: { product: Product; onSelect: (id: number) => void; manual: boolean }) {
  return (
    <button
      type="button"
    ...
    >
      ...
      {manual ? (
        <>
          {(product.languageLabel || product.rarity || product.variantLabel) && <p className="text-xs text-stone-600">{[product.languageLabel, product.rarity, product.variantLabel].filter(Boolean).join(' · ')}</p>}
          <span className="text-sm font-semibold">{product.trendPrice === undefined ? 'Cote indisponible' : euros.format(product.trendPrice)}</span>
        </>
      ) : (
